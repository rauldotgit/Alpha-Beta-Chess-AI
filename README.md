...
Mailbox uses a so called "if square attacked" function, to calculate the attacked squares of the chessboard. This process can be time consuming and leads to a significant loss in performance.
Bitboards on the other hand, operate with so called "pre calculated attack tables". The "magic Index" is implemented to reference these pre calculated attack tables, in order to obtain the data, of the attacked sqaures on the chessboard. This leads to a significant increase in performance, as a simple lookup is used, instead of real time calculations. 
